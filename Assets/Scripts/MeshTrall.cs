...
public class MeshTrall : MonoBehaviour
{
    ...
    public float mashDestoryDelay = 3.0f;               //생성된 잔상이 사라지는 데 걸리는 시간
    public Transform positonToSpawn;                    //잔상이 생성될 위치
    ...
    public float shaderVarRetreshRate = 0.05f;          //셰이더 효과가 업데이트 되는 시간 간격

    private SkinnedMeshRenderer[] skinnedRenderer;         //캐릭터의 3D 모델을 랜더링 하는 컴포넌트들
    private bool isTrailActive;                                 //현재 단상 효과가 활성화 되어 있는지 확인 하는 변수
    ...
    IEnumerator ActivateTrail(float timeActivated)      //잔상 효과 발동
    {
        ...
        while (timeActivated > 0)
        {
            ...
            if (skinnedRenderer == null)
                skinnedRenderer = positonToSpawn.GetComponentsInChildren<SkinnedMeshRenderer>();   //생성된 위치의 랜더러 컴포넌트들을 가져옴

            for(int i = 0; i < skinnedRenderer.Length; i++)                        //각 메시 렌더러에 대한 잔상 생성
            {
                GameObject gObj = new GameObject();                                     //새로운 오브젝트 생성
                gObj.transform.SetPositionAndRotation(positonToSpawn.position, positonToSpawn.rotation);
                ...
                Destroy(gObj, mashDestoryDelay);            //일정 시간 후 잔상 제거
            }
            //다음 잔상 생성까지 대기
    ...
    void Update()
    {
        if(Input.GetKeyDown(KeyCode.Space) && !isTrailActive)       //스페이스바를 부르고 현재 잔상 효과가 비활성화일 때
        {
            isTrailActive=true;
